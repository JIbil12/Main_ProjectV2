...
def generate_ecg_details(image_path):
    image = Image.open(image_path)
    current_date = datetime.now().strftime('%Y-%m-%d')
    
    prompt = f"""Analyze this ECG image and provide a detailed report (Model V2). Fill in ALL fields based on the information you can extract from the image. If you absolutely cannot determine a piece of information, state 'Unable to determine from the provided ECG image.' Do not use placeholders like '[To be filled]'. Make educated guesses where possible, but clearly indicate when you're making an assumption. Follow this structure:
...
def analyze_ecg():
    ...
    if file and allowed_file(file.filename):
        ...
        try:
            file.save(filepath)
            ecg_details = generate_ecg_details(filepath)
